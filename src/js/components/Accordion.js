...
export default class Accordion {
	...
	constructor({
		...
		this.media = media ? media : null // eslint-disable-line
	...
	onResize() {
		if (this.media) {
			if (window.innerWidth >= this.media) {
				this.isEnabled() ? this.disable() : null; // eslint-disable-line
	...
	openAccordion($parentEl, $nextElementSibling) {
		setTimeout(() => {
			if (this.closeAllAccordions === true) this.closeAllAccordion();
